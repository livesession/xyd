...
export type FlagType = 'string' | 'int' | 'float' | 'bool' | 'slice' | 'json' | 'file';

export interface PathArg {
  /** Rust local variable name for the positional (snake_case, keyword-guarded). */
  varName: string;
  /** The clap Arg id — the OpenCLI argument name, verbatim. */
  argName: string;
  wireName: string;
  idx: number;
}
...
function encodingToFlagType(encoding: string | undefined): FlagType | undefined {
  switch (encoding) {
    case 'integer':
      return 'int';
    case 'number':
...
function optionFlagType(opt: Option, encoding?: string): FlagType {
  if (!opt.arguments || opt.arguments.length === 0) return 'bool';
  const fromEncoding = encodingToFlagType(encoding);
...
export function buildLeafModel(command: Command): LeafModel {
  ...
  const pathArgs: PathArg[] = (command.arguments || []).map((arg, idx) => {
    ...
    return { varName: snakeCase(arg.name), argName: arg.name, wireName: param?.name ?? arg.name, idx };
  });
  ...
  for (const opt of command.options || []) {
    ...
    flags.push({
      ...
      wireName,
      location,
      flagType: optionFlagType(opt, encoding),
