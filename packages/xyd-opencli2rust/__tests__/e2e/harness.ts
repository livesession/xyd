...
import { buildLeafModel, type FlagType } from '../../src/model';
import { slug } from '../../src/naming';

// Reusable, self-contained end-to-end harness for opencli2rust, mirroring the
// opencli2go harness. An API's whole e2e is one call (see e2e/openai.test.ts):
...
// recorded.json = expected request); recording itself is owned by the Go
// package (E2E_RECORD there) — the fixtures here are synced copies.
...
export interface ApiConfig {
  name: string;
  cliName: string;
  /** Per-method fixtures dir: <slug>/input.json (OpenCLI) [+ output.rs, recorded.json]. */
  fixturesDir: string;
}
...
// A typed flag (int/float) rejects a non-numeric placeholder at clap's parse
...
function examplePlaceholder(flagType: FlagType | undefined): string {
  return flagType === 'int' || flagType === 'float' ? '1' : PLACEHOLDER;
}

function exampleInvocation(leaf: { command: Command; path: string[] }): { args: string[]; setFlags: Set<string> } {
  const typeByName = new Map(buildLeafModel(leaf.command).flags.map((f) => [f.flagName, f.flagType]));
  ...
  for (const o of leaf.command.options || []) {
    ...
    if (o.arguments?.length) flagArgs.push(examplePlaceholder(typeByName.get(o.name)));
  }
...
function loadPerMethod(fixturesDir: string): { slug: string; opencli: OpencliSpecJson; leaf: { command: Command; path: string[] } }[] {
  ...
  for (const slugDir of fs.readdirSync(fixturesDir).sort()) {
    const inPath = path.join(fixturesDir, slugDir, 'input.json');
    ...
    if (leaf?.command['x-openapi']) out.push({ slug: slugDir, opencli, leaf });
  }
  return out;
}
...
async function buildCli(spec: OpencliSpecJson): Promise<{ binPath: string; tmpDir: string }> {
  const tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), 'o2r-e2e-'));
  ...
  const binName = slug(spec.info?.title || 'cli') || 'cli';
  return { binPath: path.join(targetDir, 'debug', binName), tmpDir };
}
...
export function defineE2E(cfg: ApiConfig) {
  ...
  // Offline guard (no Rust toolchain): fixtures stay consistent with the OpenCLI binding.
  ...
  // Real-CLI check (gated; runs where cargo is available, e.g. CI).
  if (E2E && items.length) {
    describe(`${cfg.name} e2e: real CLI requests match fixtures`, () => {
      ...
      beforeAll(async () => {
        await server.start();
        if (!hasCargo()) {
          buildError = new Error('cargo toolchain not available');
          return;
        }
        try {
