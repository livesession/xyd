...
// e2e battery from the reusable opensdk-ruby harness: (gated) record recorded.json,
// the always-on offline request-binding guard, and (gated) the real-SDK request
// diff + the SDK's own minitest suite. Adding another complex API (stripe, github,
// …) is just dropping its per-method fixtures — no change here.
