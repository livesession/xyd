//! `xwrite_highlight` native surface (S6+ M1 H4). Replaces the codehike/
...
pub fn highlight(value: String, lang: String, meta: String, theme_json: String) -> Result<String> {
    let theme_val: serde_json::Value = serde_json::from_str(&theme_json)
        .map_err(|e| Error::from_reason(format!("[xwrite_highlight] bad theme: {e}")))?;
    ...
    let hc = match &theme_val {
        ...
        serde_json::Value::String(name) => {
            xwrite_highlight::highlighted_code(&value, &lang, &meta, name)
        }
        ...
        serde_json::Value::Object(_) => {
            ...
            let name = theme_val
                ...
                .and_then(|n| n.as_str())
                .unwrap_or("unknown");
            xwrite_highlight::highlighted_code_with_theme(&value, &lang, &meta, &theme, name)
        }
        _ => {
            return Err(Error::from_reason(
                "[xwrite_highlight] theme must be a bundled name string or a VS Code theme object",
            ))
        }
    };

    serde_json::to_string(&hc)
        .map_err(|e| Error::from_reason(format!("[xwrite_highlight] serialize: {e}")))
}
...
pub fn get_theme_colors(theme_json: String) -> Result<String> {
    ...
    let colors = match &theme_val {
        serde_json::Value::String(name) => xwrite_highlight::get_theme_colors(name),
