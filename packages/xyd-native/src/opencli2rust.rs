...
pub fn opencli2rust(spec_json: String, options_json: Option<String>) -> Result<String> {
    let spec: serde_json::Value = serde_json::from_str(&spec_json)
        .map_err(|e| Error::from_reason(format!("[opencli2rust] bad spec: {e}")))?;
    let options: Option<opencli2rust::Options> = match options_json.as_deref() {
        Some(s) => Some(
            serde_json::from_str(s)
                .map_err(|e| Error::from_reason(format!("[opencli2rust] bad options: {e}")))?,
        ),
        None => None,
    };
    let file_map = opencli2rust::opencli2rust(&spec, options);
    let entries: Vec<serde_json::Value> = file_map
        .into_iter()
        .map(|(path, entry)| {
            let write_mode = match entry.write_mode {
                opencli2rust::WriteMode::Overwrite => "overwrite",
                opencli2rust::WriteMode::SkipIfExists => "skipIfExists",
            };
            serde_json::json!({
                "path": path,
    ...
    serde_json::to_string(&entries)
        .map_err(|e| Error::from_reason(format!("[opencli2rust] serialize: {e}")))
}
