...
pub fn openapi2opencli_from_file(path: String, options_json: Option<String>) -> Result<String> {
    let options: Option<openapi2opencli::Options> = match options_json.as_deref() {
        Some(s) => Some(
            serde_json::from_str(s)
                .map_err(|e| Error::from_reason(format!("[openapi2opencli] bad options: {e}")))?,
        ),
        None => None,
    };
    let spec = openapi2opencli::openapi2opencli_from_file(&path, options)
        .map_err(|e| Error::from_reason(format!("[openapi2opencli] {e}")))?;
    serde_json::to_string(&spec)
        .map_err(|e| Error::from_reason(format!("[openapi2opencli] serialize: {e}")))
}
