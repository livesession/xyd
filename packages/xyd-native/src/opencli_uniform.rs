...
pub fn opencli_to_references_napi(
    ...
    let spec: serde_json::Value = serde_json::from_str(&spec_json)
        .map_err(|e| Error::from_reason(format!("[xyd_opencli_uniform] bad spec: {e}")))?;

    let mut options = OpencliToReferencesOptions::default();
    if let Some(raw) = options_json {
        let parsed: serde_json::Value = serde_json::from_str(&raw)
            .map_err(|e| Error::from_reason(format!("[xyd_opencli_uniform] bad options: {e}")))?;
    ...
    serde_json::to_string(&refs)
        .map_err(|e| Error::from_reason(format!("[xyd_opencli_uniform] serialize: {e}")))
}
