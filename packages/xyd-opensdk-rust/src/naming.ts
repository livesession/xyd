...
/** SCREAMING_SNAKE_CASE for consts / enum member docs, e.g. "petStatus" -> "PET_STATUS". */
...
/** A Cargo package / lib crate name (snake_case, alnum + underscore), e.g. "My API" -> "my_api". */
...
/**
 * The idiomatic Rust method name for an IR action. openai keeps the verb as-is
 * (`create`, `retrieve`, `list`, `update`, `delete`), so a snake_cased action is
 * already the native name; keyword collisions are guarded.
 */
export function rustMethodName(action: string): string {
