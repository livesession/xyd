...
/** The single top-level resource `.rs` file in a generated project (the interesting bit). */
function resourceFileKey(files: Record<string, string>): string | undefined {
  const runtime = new Set(['src/lib.rs', 'src/client.rs', 'src/models.rs', 'src/error.rs', 'src/transport.rs']);
  return Object.keys(files).find((k) => /^src\/[^/]+\.rs$/.test(k) && !runtime.has(k));
}

// ---- Generator (opt-in): one CORRECT fixture per real spec operation ------
describe.runIf(BUILD && fs.existsSync(SPEC))('generate opensdk -> rust fixtures (assumed-correct, for review)', () => {
  ...
  it('build __fixtures__/-2.complex.openai/<op>/{input.json, output.rs}', () => {
    ...
    for (const { path: p, method, operation } of eachOperation(spec)) {
      ...
      fs.writeFileSync(path.join(outDir, 'output.rs'), files[key]);
      n++;
    }
...
// ---- Regen guard (offline; pure opensdk IR -> rs) ------------------------
const fixtures = fs.existsSync(O2S_FIX)
  ? fs.readdirSync(O2S_FIX).filter((d) => fs.existsSync(path.join(O2S_FIX, d, 'input.json')))
  : [];

describe.skipIf(!fixtures.length || BUILD)('opensdk-rust docs (IR -> rs, regen guard)', () => {
  for (const dir of fixtures) {
    it(dir, () => {
      const ir = JSON.parse(fs.readFileSync(path.join(O2S_FIX, dir, 'input.json'), 'utf8'));
      const files = opensdkRust(ir);
      const key = resourceFileKey(files);
      expect(key, `no resource .rs generated for ${dir}`).toBeTruthy();
      const expected = fs.readFileSync(path.join(O2S_FIX, dir, 'output.rs'), 'utf8');
