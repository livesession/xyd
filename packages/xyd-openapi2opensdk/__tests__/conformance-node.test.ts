...
// ../oracle/parseOpenaiNode is gitignored/encrypted - imported LAZILY in the refresh block below so this suite SKIPS (not fails to load) when the oracle plaintext is absent (CI without XYD_CONTENT_SECRET, fresh clones, fork PRs).
...
// openai-node resource classes live under src/resources/**; the root resource
// graph is on `class OpenAI` in src/client.ts, wired through the re-export barrel
// src/resources/index.ts (kept so the ctor references can be resolved).
function isNodeSource(p: string): boolean {
  if (!p.endsWith('.ts') || p.endsWith('.test.ts')) return false;
  return p === 'src/client.ts' || p.startsWith('src/resources/');
}

async function fetchOpenaiNode(): Promise<{ sha: string; files: Record<string, string> }> {
  ...
  const paths: string[] = (tree.tree as Array<{ path: string; type: string }>)
    .filter((e) => e.type === 'blob' && isNodeSource(e.path))
    .map((e) => e.path);
...
describe.skipIf(!REFRESH)('node oracle refresh (network)', () => {
  it('fetch + parse openai-node → oracle/node-surface.json', async () => {
    const { sha, files } = await fetchOpenaiNode();
    const { parseOpenaiNode } = await import('../oracle/parseOpenaiNode');
    const surface = parseOpenaiNode(files);

    fs.mkdirSync(ORACLE_DIR, { recursive: true });
    fs.writeFileSync(SURFACE, `${JSON.stringify(surface, null, 2)}\n`);
    fs.writeFileSync(
      path.join(ORACLE_DIR, 'node-pins.json'),
      `${JSON.stringify(
...
// ---- Conformance (offline; reads committed node-surface.json) -------------
const hasOracle = fs.existsSync(SURFACE);

describe.skipIf(!hasOracle)('conformance: openapi → opensdk surface vs openai-node', () => {
  it('matches the oracle method surface above the coverage floor', async () => {
    const oracle = JSON.parse(fs.readFileSync(SURFACE, 'utf8'));
    // openai-node surface is L0-only (no params parsed), so ignore param/response L1 noise.
    ...
    console.log(
      `[conformance:node] L0 ${(diff.l0Coverage * 100).toFixed(1)}%  ` +
