...
export function splitWords(input: string): string[] {
  return input
    ...
    .split(/[^a-zA-Z0-9]+/) // ANY non-alphanumeric — wire junk (`ids[]`) must never leak into a name
...
/** PascalCase a wire name (e.g. `chat_completion` -> `ChatCompletion`). */
...
/**
 * Slugify a free-form title into a package/binary name (lowercase, dash
 * separated, alnum only). e.g. "OpenAI API" -> "openai-api".
 */
