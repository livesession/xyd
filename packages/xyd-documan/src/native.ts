...
function load(): any | null {
    ...
    if (process.env?.XYD_REQUIRE_NATIVE === "1" && typeof mod?.getThemeColors !== "function") {
        throw new Error("XYD_REQUIRE_NATIVE=1 but @xyd-js/native.getThemeColors is unavailable");
    }
    return mod;
}
