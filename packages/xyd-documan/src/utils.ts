...
// TODO: better + faster check
const resolveComponentDist = (dist: string): { resolvedDist: string; exists: boolean } => {
...
function cloneComponentsPreservingReferences(componentPlugins: any[]): any[] {
    return componentPlugins.map((comp) => {
        // Create a shallow clone of each component object
        const cloned = { ...comp };

        // Preserve function/object references by keeping them as-is
        // Only clone primitive values and arrays/objects that don't contain functions
        for (const key in cloned) {
            if (cloned.hasOwnProperty(key)) {
                const value = cloned[key];

                // Keep functions and component references as-is
                ...
                // For plain objects (but not React components or class instances), create shallow copy
                else if (
                    value &&
                    typeof value === "object" &&
                ...
                // Primitives (strings, numbers, booleans) are copied by value automatically
            }
        }
                ...
                if (Array.isArray(p.components)) {
                    for (const component of p.components) {
                        ...
                        if (!component.dist) {
                            ...
                            const looksLikeFilePath = /\.[tj]sx?$/.test(path.basename(pluginPkg));
                            const pluginPkgBase = looksLikeFilePath
                                ? path.dirname(pluginPkg)
                                : pluginPkg.replace(/\/[^\/]+\.(ts|js|tsx|jsx|mjs|cjs)$/, "");

                            // Use component function name instead of the key name
    ...
    return {
        ...
        async load(id) {
            if (id === "virtual:xyd-user-components.jsx") {
                ...
                const resolveExistingDist = (dist?: string) => {
                ...
                // Generate imports only for non-inline components (preserve original index)
                const imports = userComponents
                    .map((component, index) => {
                        const dist = component?.dist;
                        const explicitInline = component?.isInline === true;
                        const looksLikePath =
                            typeof dist === "string" && /^([./]|[A-Za-z]:)/.test(dist);
...
function integrationsToPlugins(integrations: Integrations) {
    ...
    if (
        ...
        typeof integrations?.diagrams === "object" &&
        ".config" in integrations?.diagrams
    ) {
