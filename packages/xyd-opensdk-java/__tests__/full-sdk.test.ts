...
// The ENTIRE produced Java SDK, assembled by merging every committed per-method
// IR into one document and running opensdkJava. This is the whole thing —
// Client.java, every <Resource>Service.java + params/model POJOs, the enums,
// the page containers, the dependency-free runtime, and the generated
// <Resource>ServiceTest.java suite — committed as a golden so we can diff the
// complete generated SDK (not just one file per method). Regenerate with
// O2S_BUILD_DOCS=1; javac-compile the whole tree with O2S_JAVA_SMOKE=1.
...
const generate = () => opensdkJava(fullIR(PER_METHOD, 'openai'));

// ---- Generator (opt-in) --------------------------------------------------
describe.runIf(BUILD)('generate the entire Java SDK golden', () => {
...
// ---- Regen guard (offline) ----------------------------------------------
describe.skipIf(!fs.existsSync(OUT) || BUILD)('opensdk-java entire SDK (whole tree, regen guard)', () => {
...
describe.runIf(JAVA_SMOKE && fs.existsSync(OUT))('opensdk-java entire SDK (javac over all sources)', () => {
