...
export function generateAuthPrehydrationScript(
  ...
  const script = `(function(){var n="%%COOKIE_NAME%%";var t=null;try{t=localStorage.getItem(n)}catch(e){}if(!t){var c=document.cookie.split(";");for(var i=0;i<c.length;i++){var p=c[i].trim().split("=");if(p[0]===n+"-state"){t=decodeURIComponent(p[1]);break}}}var a=false;var g=[];if(t){try{var d=JSON.parse(atob(t.split(".")[1]));if(d.exp&&d.exp*1000>Date.now()){a=true;g=d["%%GROUPS_CLAIM%%"]||[]}}catch(e){}}window.__xydAuthState={authenticated:a,groups:g,token:a?t:null};document.documentElement.setAttribute("data-auth",a?"authenticated":"anonymous")})();`;
