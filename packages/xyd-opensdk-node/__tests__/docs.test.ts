...
describe.runIf(BUILD)('generate opensdk → node fixtures (assumed-correct, for review)', () => {
  for (const src of CORPUS_SPECS) {
    ...
    it(`build ${src.slug}/<op>/{input.json, output.ts}`, () => {
...
describe.skipIf(!anyFixtures || BUILD)('opensdk-node docs (IR → node, regen guard)', () => {
  for (const corpus of corpora) {
    for (const dir of corpus.dirs) {
      it(`${corpus.name}/${dir}`, () => {
        ...
        const expected = fs.readFileSync(path.join(corpus.dir, dir, 'output.ts'), 'utf8');
