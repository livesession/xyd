const O = globalThis, D = O.ShadowRoot && (O.ShadyCSS === void 0 || O.ShadyCSS.nativeShadow) && "adoptedStyleSheets" in Document.prototype && "replace" in CSSStyleSheet.prototype, j = /* @__PURE__ */ Symbol(), I = /* @__PURE__ */ new WeakMap();
let et = class {
  constructor(t, e, s) {
...
const ht = (r) => new et(typeof r == "string" ? r : r + "", void 0, j), at = (r, ...t) => {
...
const { is: ct, defineProperty: dt, getOwnPropertyDescriptor: pt, getOwnPropertyNames: ut, getOwnPropertySymbols: $t, getPrototypeOf: _t } = Object, R = globalThis, q = R.trustedTypes, ft = q ? q.emptyScript : "", mt = R.reactiveElementPolyfillSupport, b = (r, t) => r, M = { toAttribute(r, t) {
  switch (t) {
    case Boolean:
      r = r ? ft : null;
      break;
...
y.elementStyles = [], y.shadowRootOptions = { mode: "open" }, y[b("elementProperties")] = /* @__PURE__ */ new Map(), y[b("finalized")] = /* @__PURE__ */ new Map(), mt?.({ ReactiveElement: y }), (R.reactiveElementVersions ??= []).push("2.1.2");
const z = globalThis, Z = (r) => r, H = z.trustedTypes, F = H ? H.createPolicy("lit-html", { createHTML: (r) => r }) : void 0, st = "$lit$", _ = `lit$${Math.random().toFixed(9).slice(2)}$`, it = "?" + _, At = `<${it}>`, A = document, w = () => A.createComment(""), C = (r) => r === null || typeof r != "object" && typeof r != "function", V = Array.isArray, yt = (r) => V(r) || typeof r?.[Symbol.iterator] == "function", k = `[ 	
\f\r]`, E = /<(?:(!--|\/[^a-zA-Z])|(\/?[a-zA-Z][^>\s]*)|(\/?$))/g, J = /-->/g, G = />/g, f = RegExp(`>|${k}(?:([^\\s"'>=/]+)(${k}*=${k}*(?:[^ 	
\f\r"'\`<>=]|("|')|))|$)`, "g"), Y = /'/g, Q = /"/g, rt = /^(?:script|style|textarea|title)$/i, gt = (r) => (t, ...e) => ({ _$litType$: r, strings: t, values: e }), Rt = gt(1), g = /* @__PURE__ */ Symbol.for("lit-noChange"), d = /* @__PURE__ */ Symbol.for("lit-nothing"), X = /* @__PURE__ */ new WeakMap(), m = A.createTreeWalker(A, 129);
function nt(r, t) {
...
const vt = (r, t) => {
  ...
  for (let a = 0; a < e; a++) {
    const h = r[a];
    let c, p, l = -1, u = 0;
    for (; u < h.length && (o.lastIndex = u, p = o.exec(h), p !== null); ) u = o.lastIndex, o === E ? p[1] === "!--" ? o = J : p[1] !== void 0 ? o = G : p[2] !== void 0 ? (rt.test(p[2]) && (i = RegExp("</" + p[2], "g")), o = f) : p[3] !== void 0 && (o = f) : o === f ? p[0] === ">" ? (o = i ?? E, l = -1) : p[1] === void 0 ? l = -2 : (l = o.lastIndex - p[2].length, c = p[1], o = p[3] === void 0 ? f : p[3] === '"' ? Q : Y) : o === Q || o === Y ? o = f : o === J || o === G ? o = E : (o = f, i = void 0);
    const $ = o === f && r[a + 1].startsWith("/>") ? " " : "";
    n += o === E ? h + At : l >= 0 ? (s.push(c), h.slice(0, l) + st + h.slice(l) + _ + $) : h + _ + (l === -2 ? a : $);
  }
    ...
    for (; (i = m.nextNode()) !== null && h.length < a; ) {
      if (i.nodeType === 1) {
        if (i.hasAttributes()) for (const l of i.getAttributeNames()) if (l.endsWith(st)) {
          ...
          h.push({ type: 1, index: n, name: U[2], strings: $, ctor: U[1] === "." ? bt : U[1] === "?" ? St : U[1] === "@" ? wt : N }), i.removeAttribute(l);
...
Pt?.(P, x), (z.litHtmlVersions ??= []).push("3.3.2");
const xt = (r, t, e) => {
...
const Ot = { attribute: !0, type: String, converter: M, reflect: !1, hasChanged: L }, Tt = (r = Ot, t, e) => {
...
const T = "2.5.1", tt = "__vscodeElements_disableRegistryWarning__", ot = (r, t) => {
  console.warn(t ? `[VSCode Elements] ${r}
%o` : `${r}
%o`, t);
};
class kt extends S {
...
const Dt = (r) => (t) => {
  ...
  if (tt in window)
    return;
  const i = document.createElement(r)?.version;
  let n = "";
  i ? i !== T ? (n += "is already registered by a different version of VSCode Elements. ", n += `This version is "${T}", while the other one is "${i}".`) : n += `is already registered by the same version of VSCode Elements (${T}).` : n += "is already registered by an unknown custom element handler class.", ot(`The custom element "${r}" ${n}
To suppress this warning, set window.${tt} to true`);
}, jt = at`
  :host([hidden]) {
    display: none;
  }
