...
|----------|-------------|
| `"node"` | Standalone Node.js server (auto-generated by `xyd build`) |
| `"netlify"` | Netlify Edge Functions |
| `"vercel"` | Vercel Routing Middleware |
| `"cloudflare"` | Cloudflare Pages Functions |

### Node.js Server

With `edge.platform: "node"`, `xyd build` generates a `server.mjs` in your build output:
...
| **Layer 2** (edge) | Server-side JWT verification. Content never served to unauthorized. | Node.js, Netlify, Vercel, Cloudflare |

## CLI Commands

| Command | Description |
|---------|-------------|
| `xyd build` | Builds static site. With `edge.platform: "node"`, generates `server.mjs`. |
| `xyd serve` | Serves the built site in production. Auto-detects edge mode. |
