...
export function Navbar() {
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      const navbarHeight = 100; // Approximate navbar position from top

      // Find which section is at the navbar position
      for (const section of Array.from(sections)) {
        const rect = section.getBoundingClientRect();

        // Check if this section overlaps with the navbar position
  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 my-4 md:my-8 px-6 lg:px-8">
      <div ref={navRef} className="mx-auto max-w-[calc(100%-3rem)] xl:max-w-[1600px]">
        ...
        <div className="flex h-14 items-center justify-between">
            ...
            >
              <g clipPath="url(#clip0_3_18)">
                <path
                  ...
                  fill={isDarkSection ? "white" : "#111827"}
                  stroke={isDarkSection ? "white" : "#111827"}
          ...
          <div className="hidden lg:flex items-center gap-3 xl:gap-4">
            ...
            <button className="rounded-md bg-white px-3 xl:px-4 py-1 text-sm xl:text-base font-normal text-gray-900 shadow-sm hover:bg-gray-100 transition-colors whitespace-nowrap">
              Download (soon)
            </button>
          </div>
          ...
          <button
            ...
            className={`lg:hidden transition-colors duration-500 ${isDarkSection ? "text-white" : "text-gray-900"}`}
          >
            {isMobileMenuOpen ? (
              <svg
                className="w-6 h-6"
        ...
        {isMobileMenuOpen && (
          <div className="lg:hidden py-4 border-t border-gray-700/20 mt-2">
            <div className="flex flex-col gap-4 text-sm">
              ...
              <button className="rounded-md bg-white px-4 py-2 text-md font-normal text-gray-900 shadow-sm hover:bg-gray-100 transition-colors text-left">
                Download (soon)
              </button>
            </div>
          </div>
