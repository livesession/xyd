...
export const config = {
   ...
   * "Publish" only). The dev plugin sets it to a host.docker.internal URL so
   * in-container Gitea can reach the host-port gateway. */
  platformPublicUrl: process.env.PLATFORM_PUBLIC_URL ?? "",
  /** Public registry host baked into an SDK's `sdk.json` `spec` reference (e.g.
   * "registry.apitoolchain.dev"). The ref is `[<host>/]apis/<ns>/<api>@<ver>`;
   * empty in dev → a clean, host-less ref (`apis/<ns>/<api>@<ver>`). */
  platformRegistryHost: process.env.PLATFORM_REGISTRY_HOST ?? "",
