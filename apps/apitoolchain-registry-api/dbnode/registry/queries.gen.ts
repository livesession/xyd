...
import type {
  ListApisRow,
  ListApisByProjectArgs,
  ListApisByProjectRow,
  GetApiArgs,
  GetApiRow,
  UpsertApiArgs,
  UpsertApiRow,
  ...
  GetVersionArgs,
  GetVersionRow,
  GetCurrentVersionArgs,
  GetCurrentVersionRow,
  ClearCurrentVersionsArgs,
  SetCurrentVersionArgs,
  UpsertVersionArgs,
  UpsertVersionRow,
  ListDistTagsArgs,
  ListDistTagsRow,
...
export const updateApiQuery = `-- name: UpdateApi :one
UPDATE apis SET name = $2, description = $3, updated_at = now()
WHERE id = $1
RETURNING id, name, description, format, namespace, source, created_at, updated_at, kind, project_id`;

export async function updateApi(client: Client, args: UpdateApiArgs): Promise<UpdateApiRow | null> {
    const result = await client.query({
        text: updateApiQuery,
