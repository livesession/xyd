//! Tier-1 full-tree golden parity: generate_rust(input.json) === the committed
//! output/ tree, byte-exact. Now that the emitter produces the FULL tree (adds
//! the vendored reqwest/tokio runtime + the #[tokio::test] suite), each fixture is
//! checked three ways: (a) every golden file under output/ is emitted and
//! byte-exact, (b) every emitted file has a matching golden (no extras), and (c) a
