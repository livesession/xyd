    ...
    def request(method:, path:, query: nil, body: nil, headers: nil, encoding: "json", raw: false, idempotency: false)
      ...
      unless body.nil?
        ...
        body.each do |key, value|
          payload[key] = Openai.encode(value) unless value.nil?
        end
        if encoding == "multipart"
          data, content_type = Openai.multipart_body(payload)
    ...
    def perform(method, url, headers, data, raw)
      ...
      loop do
        ...
        if status >= 400
          ...
          raise Openai.error_for_status(status, response_headers, response.body)
        end
