...
module Petstore
  ...
  USER_AGENT = "petstore-ruby/1.2.0"
    ...
    def request(method:, path:, query: nil, body: nil, headers: nil, encoding: "json", raw: false, idempotency: false)
      ...
      unless body.nil?
        ...
        body.each do |key, value|
          payload[key] = Petstore.encode(value) unless value.nil?
        end
        if encoding == "multipart"
          data, content_type = Petstore.multipart_body(payload)
    ...
    def perform(method, url, headers, data, raw)
      ...
      loop do
        ...
        if status >= 400
          ...
          raise Petstore.error_for_status(status, response_headers, response.body)
        end
