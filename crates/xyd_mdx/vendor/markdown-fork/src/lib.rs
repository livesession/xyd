...
#![no_std]
// xyd C-S2: vendored fork of markdown-rs 1.0.0. Upstream ran `deny(pedantic)` on
// its own pinned toolchain; on this repo's (newer) toolchain both untouched
// upstream files and the added directive constructs trip newer pedantic/rustc
// lints. Vendored third-party code is not held to this repo's `-D warnings`
// gate — the linting that matters is on the first-party `xyd_mdx` crate.
#![allow(warnings)]
#![allow(clippy::all)]
