...
fn exit(context: &mut CompileContext) -> Result<(), message::Message> {
    match context.events[context.index].name {
        ...
        Name::DirectiveLeafName => on_exit_directive_leaf_name(context),
        Name::DirectiveLeafAttributesContent => on_exit_directive_leaf_attributes(context),
        Name::DirectiveContainerName => on_exit_directive_container_name(context),
        Name::DirectiveContainerAttributesContent => {
...
// SPIKE: remark-directive node builders.
fn on_enter_leaf_directive(context: &mut CompileContext) {
...
fn on_enter_container_directive(context: &mut CompileContext) {
...
fn on_exit_directive_leaf_name(context: &mut CompileContext) {
...
fn on_exit_directive_leaf_attributes(context: &mut CompileContext) {
    let value = Slice::from_position(
        context.bytes,
        &SlicePosition::from_exit_event(context.events, context.index),
    )
...
fn on_exit_directive_container_name(context: &mut CompileContext) {
    let value = Slice::from_position(
        context.bytes,
        &SlicePosition::from_exit_event(context.events, context.index),
    )
    .serialize();
    if let Node::ContainerDirective(node) = context.tail_mut() {
        node.name = value;
    } else {
        unreachable!("expected container directive on stack");
    }
}

fn on_exit_directive_container_attributes(context: &mut CompileContext) {
    let raw = Slice::from_position(
        context.bytes,
        &SlicePosition::from_exit_event(context.events, context.index),
    )
    .serialize();
    // The container header token includes the surrounding braces (`{a=1}`);
    // strip them to match the leaf's inner-attributes representation.
...
fn on_exit_directive_container_content(context: &mut CompileContext) {
    let value = Slice::from_position(
        context.bytes,
        &SlicePosition::from_exit_event(context.events, context.index),
    )
