...
/// <summary>The typed exception for the Client error kind.</summary>
public sealed class ClientException : ApiException
{
    public ClientException(int statusCode, string? responseBody, string? requestId, string message)
        : base(statusCode, responseBody, requestId, message)
    {
    }

    /// <inheritdoc/>
    public override string Kind => "Client";
}
...
/// <summary>The typed exception for the Missing error kind.</summary>
public sealed class MissingException : ApiException
{
    public MissingException(int statusCode, string? responseBody, string? requestId, string message)
        : base(statusCode, responseBody, requestId, message)
    {
    }
...
/// <summary>The typed exception for the Server error kind.</summary>
public sealed class ServerException : ApiException
{
    public ServerException(int statusCode, string? responseBody, string? requestId, string message)
        : base(statusCode, responseBody, requestId, message)
    {
    }

    /// <inheritdoc/>
    public override string Kind => "Server";
}
...
internal sealed class Transport
{
    ...
    private const string DefaultBaseUrl = "https://api.uniondepot.test/v1";
    private const string UserAgentIdentifier = "x-UnionDepot-dotnet/0.4.0";
    private static readonly bool IncludeRuntimeVersion = true;
    private const int DefaultTimeoutMs = 1234;
    private const string TimeoutEnvVar = "WIRE_TIMEOUT_MS";
    private const int MaxRetries = 5;
    private static readonly bool RetryConnectionErrors = false;
    private static readonly bool HonorRetryAfterHeader = false;
    private const double BackoffInitialMs = 100.0;
    private const double BackoffMaxMs = 2000.0;
    private const double BackoffMultiplier = 3.0;
    private const double BackoffJitter = 0.0;
    private const string RequestIdHeader = "X-Req";
    private const string IdempotencyHeader = "X-Idem";
    private static readonly HashSet<int> RetryableStatusCodes = new() { 429 };
    ...
    private static ApiException BuildError(HttpResponseMessage response, string body)
    {
        ...
        return status switch
        {
            ...
            404 => new MissingException(status, body, requestId, message),
            ...
            _ => status >= 500 ? new ServerException(status, body, requestId, message) : new ClientException(status, body, requestId, message),
        };
    }
