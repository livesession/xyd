					...
					&cli.Command{
						...
						Commands: []*cli.Command{
							&cli.Command{
								...
								Action: handleOrganizationProjectsCertificatesList,
							},
						},
					},
				},
...
func handleOrganizationProjectsCertificatesList(ctx context.Context, cmd *cli.Command) error {
	projectID := cmd.Args().Get(0)
	path := "/organization/projects/" + url.PathEscape(projectID) + "/certificates"
	query := url.Values{}
	if cmd.IsSet("limit") {
