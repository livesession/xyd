...
| `XYD_BASENAME` | string | undefined | Mount path fallback for `advanced.basename` (set by 3rd-party orchestrators like `@xyd-js/vite-plugin`); an explicit `advanced.basename` in settings wins | Build/Dev |
| `XYD_LIVERELOAD` | `0` | undefined | `0` disables the bun dev server's livereload websocket client (set by embedding hosts whose proxy can't forward upgrades — nuxt, Next) | Development |
