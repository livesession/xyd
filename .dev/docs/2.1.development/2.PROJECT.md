...
├── crates/                         # Rust workspace (rooted here, not at the repo root)
├── xwrite/                         # Submodule: content engine (livesession/xwrite)
├── opensdk/                        # Submodule: SDK/CLI toolchain (livesession/opensdk)
└── .xyd/
    └── host/                       # Runtime host (generated)
```

Both submodules are path-dependencies of the Rust build, not optional extras: `crates/` and
`packages/xyd-native` path-dep them, so without a checkout cargo cannot load the workspace at
all. `scripts/setup.sh` initializes them (`git submodule update --init xwrite opensdk apitoolchain`) and
`scripts/ensure-submodules.mjs` is the preflight that reports every missing one in one message.
...
| `crates/` | xyd's Rust crates (CLI, converters, settings, watch) | `crates/Cargo.toml` |
| `xwrite/` | Content engine: frontmatter, highlight, math, mdx | `xwrite/Cargo.toml` |
| `opensdk/` | SDK/CLI toolchain: converters, the seven emitters, the `opensdk` binary | `opensdk/Cargo.toml` |
| `apitoolchain/` | API converters (uniform/openapi/gql/mcp/opencli) + their five npm shims + the `api` CLI | `apitoolchain/Cargo.toml` |
